...
class RolodexPlugin {
  ...
  constructor(node, peerCacheFilePath) {
    ...
    this.node.router.events.on('remove', identity => {
      this.node.logger.debug(`removeing peer ${identity}`)
      this.removeContact(identity)
    });
  }
  ...
  removeContact(identity) {
    this.db.remove({ identity }, { multi: true }, err => {
      if (err) {
        this.node.logger.error(`failed to remove peer ${identity}`)
      }
    })
  }
